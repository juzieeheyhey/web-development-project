...
export const createEventPermission = (req: Request, res: Response, next: NextFunction) => {
  const authHeader = req.headers.authorization;
  ...
  try {
    const decoded = jwt.verify(token, env.JWT_TOKEN_SECRET);
    req.body.user = decoded;
    const canPostEvents = req.body.user.canPostEvents;


    if (!canPostEvents) {
      // If the user doesn't have permission, handle the case where they access the /events/create URL

      return res.status(403).send('Access Denied');
    }
    // check if has event create permission
  } catch (err) {
    return res.status(401).send('Invalid Token');
  }
